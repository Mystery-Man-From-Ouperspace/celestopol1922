...
export class CEL1922CharacterSheet extends CEL1922ActorSheet {
  ...
  */
  ...
  _onDragStart(event) {
    ...
    if (target.classList.contains("lancerDeDes")) {

      const type = "celestopol1922.lancerDeDes"

      const whatIsIt = target.dataset.libelId;                   // Va récupérer 'artifice-AME-1' par exemple
      ...
      // let valeur = this.actor.system.harmoniques[dice].valeur
      dragData = {
        type,
...
async function _onClickDiceRollFollow(myActor, skillNumUsedLibel) {
  ...
  if (myData.mySkill === 0 || myData.mySkill === 5 || myData.mySkill === 10 || myData.mySkill === 15) { // Si c'est un test de résistance
    ...
    myData.totalBoni = myData.myValue + myData.myWoundsMalus - myData.myArmorVal;
